...
<<<<<<< HEAD
                    ...
                    if(!empty($sub)){
                        ...
                        usort($sub, function($a, $b) {
                            $a_id = isset($a['set_id']) ? $a['set_id'] : 0;
                            $b_id = isset($b['set_id']) ? $b['set_id'] : 0;
                            return $a_id - $b_id;
                        });
                        ...
                        foreach($sub as $value){
                            ...
                            } elseif(!empty($value['return_details'])){
                                if(is_array($value['return_details'])){
                                    $remarks = isset($value['return_details']['return_remarks']) 
                                        ? $value['return_details']['return_remarks'] : '';
                                } else {
                                    $decoded = json_decode($value['return_details'], true);
                                    if(is_array($decoded) && isset($decoded['return_remarks'])){
                                        $remarks = $decoded['return_remarks'];
                                    }
                                }
                            }
                            ...
                            // ===== SAFE DATES =====
                            $date_issued = '';
                            if(!empty($value['date_issued'])){
                                $date_issued = $value['date_issued'];
                            } elseif(!empty($value['et_details'])){
                                if(is_array($value['et_details']) && isset($value['et_details']['date_issued'])){
                                    $date_issued = $value['et_details']['date_issued'];
                                } else {
                                    $decoded = json_decode($value['et_details'], true);
                                    if(is_array($decoded) && isset($decoded['date_issued'])){
                                        $date_issued = $decoded['date_issued'];
                                    }
                                }
                            }

                            $date_returned = isset($value['date_returned']) ? $value['date_returned'] : '';

                            // ===== UNIQUE KEY (FIX DUPLICATES) =====
                            $key = $value['ed_id'].'_'.$value['et_id'].'_'.$remarks;

                            if(!isset($data2[$key])){
                                $data2[$key] = array(
                                    'set_id' => isset($value['set_id']) ? $value['set_id'] : 0,
                                    'count_set' => isset($value['count_set']) ? $value['count_set'] : 1,
                                    ...
                                    'set_price' => isset($value['set_price']) ? $value['set_price'] : 0,
                                    'set_currency' => isset($value['set_currency']) ? $value['set_currency'] : '',
                                    'lost' => isset($value['lost']) ? $value['lost'] : 0,
                    ...
                    ?>
                    <tr style="<?php echo ($det['lost']!=0) ? 'background-color:#ec7070;' : ''; ?>">
                        ...
                        <!-- DATE ISSUED -->
                        <td class="main-tab" align="center">
                            <?php echo !empty($det['date_issued']) ? $det['date_issued'] : '-'; ?>
                        </td>
                        ...
                        <?php if ($det['set_id']!=0 && ($previousId !== $det['set_id'])) { ?>
                            <td class="main-tab" align="center" rowspan="<?php echo $det['count_set']; ?>">
                                <?php echo $det['set_price']." <small>".$det['set_currency']."</small>"; ?>
                            </td>
                        <?php } else { ?>
                            <td class="main-tab" align="center">
                                <?php echo $det['unit_price']." <small>".$det['currency']."</small>"; ?>
                            </td>
                        <?php } ?>
                    ...
                    ?>
                    <tr>
                        <td class="main-tab" align="center" colspan="9">No Data Available.</td>
                    </tr>
                    <?php } ?>
