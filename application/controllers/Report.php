...
class Report extends CI_Controller {
    ...
    public function export_overall(){
        ...
        if($filters!=''){
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh LEFT JOIN et_details ed ON eh.et_id = ed.et_id WHERE cancelled='0' AND ".$query) AS $et){
                ...
                if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade==0 && $et->damage==0 && $et->obsolete==0){
                    $status = $employee;
                }else if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade!=0 && $et->damage==0){
                    $status = $employee." / Upgraded";
                }else if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade!=0 && $et->damage==1){
                    $status = $employee." / Upgraded / Damaged";
                }else if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade==0 && $et->damage==1){
                    $status = $employee." / Damaged";
            ...
            foreach($this->super_model->select_custom_where("et_head","cancelled='0' ORDER BY et_desc ASC") AS $ss){
                foreach($this->super_model->select_row_where("et_details","et_id",$ss->et_id) AS $r){
                    ...
                    if($ss->accountability_id!=0 && $r->borrowed==0 && $r->lost==0 && $r->upgrade==0 && $r->damage==0 && $r->obsolete==0){
                    $status = $employee;
                    }else if($ss->accountability_id!=0 && $r->borrowed==0 && $r->lost==0 && $r->upgrade!=0 && $r->damage==0){
                        $status = $employee." / Upgraded";
                    }else if($ss->accountability_id!=0 && $r->borrowed==0 && $r->lost==0 && $r->upgrade!=0 && $r->damage==1){
                        $status = $employee." / Upgraded / Damaged";
                    }else if($ss->accountability_id!=0 && $r->borrowed==0 && $r->lost==0 && $r->upgrade==0 && $r->damage==1){
                        $status = $employee." / Damaged";
                    }else if($ss->accountability_id==0 && $r->damage==0 && $r->change_location==0 && $r->upgrade!=0){
                        $status = 'Available / Upgraded';
                    }else if($ss->accountability_id==0 && $r->damage==0 && $r->change_location==0 && $r->upgrade==0){
                        $status = 'Available';
                    }else if($ss->accountability_id==0 && $r->change_location==1){
                        ...
                        $status = 'Borrowed';
                    }else if($r->damage==1 && $ss->accountability_id==0){
                        $status = 'Damaged';
                    }else if($r->damage==1 && $ss->accountability_id!=0){
                        $status = 'Damaged / '.$employee;
                    }else if($r->lost==1){
    ...
    public function inv_rep_det(){  
        ...
        if($row!=0){
            foreach($this->super_model->select_row_where("et_head","subcat_id",$subcat) AS $t){
                ...
                $obsolete = $this->super_model->select_column_where("et_details", "obsolete", "et_id", $t->et_id); 
    ...
    public function report_main(){  
        ...
        if($row!=0){
            foreach($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id = ed.et_id WHERE accountability_id!='0' AND cancelled='0' AND save_temp='0' AND lost='0' AND obsolete='0' GROUP BY eh.et_id") AS $et){
    ...
    public function insert_obsolete(){
        $obsolete_date = trim($this->input->post('obsolete_date')," ");
        $obsolete_remarks = trim($this->input->post('obsolete_remarks')," ");
        $obsolete_accountable = trim($this->input->post('obsolete_accountable')," ");
        $ed_id = trim($this->input->post('ed_id')," ");
        $data_obsolete = array(
            'obsolete'=>1,
            'obsolete_date'=>$obsolete_date,
            'obsolete_remarks'=>$obsolete_remarks,
            'obsolete_accountable'=>$obsolete_accountable,
        );

        if($this->super_model->update_where('et_details', $data_obsolete, 'ed_id', $ed_id)){
            echo "<script>alert('Successfully Tagged as Obsolete!'); location.replace(document.referrer); </script>";
        }
    }
    ...
    public function report_print(){  
        ...
        // if($filters!=''){
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id = ed.et_id WHERE eh.accountability_id!='0' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND obsolete='0' $filter_query GROUP BY eh.et_id ORDER BY ed.set_id DESC") AS $et){
                $data['user_id'] =$_SESSION['fullname'];
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
        ...
        // }else {
        //     foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id = ed.et_id WHERE accountability_id!='0' AND save_temp='0' AND lost='0' AND obsolete='0' GROUP BY eh.et_id") AS $et){
        //         $data['user_id'] =$_SESSION['fullname'];
        //         $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
        //         $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
        //         $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
        ...
        //         $set_name = $this->super_model->select_column_where("et_set", "set_name", "set_id", $et->set_id);
        //         $set_serial = $this->super_model->select_column_where("et_set", "set_serial_no", "set_id", $et->set_id);
        //         $count_set = $this->super_model->count_custom("SELECT eh.et_id FROM et_details ed INNER JOIN et_head eh ON eh.et_id = ed.et_id WHERE accountability_id!='0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND save_temp='0' AND set_id ='$et->set_id'");
        //         $count_set_id = $this->super_model->count_rows_where("et_details","set_id",$et->set_id);
        //         if(empty($set_name)){
        //             $unit_price=(float)$et->unit_price;
        //             $total = $et->qty*(float)$unit_price;
        ...
        //             'set_name'=>$set_name,
        //             'set_serial'=>$set_serial,
        //             'inspection_date'=>implode("\n",$inspection_date),
        //             'inspected_by'=>implode("\n",$inspected_by),
        //             'ins_status'=>implode("\n",$ins_status),
        //             'ins_remarks'=>implode("\n",$ins_remarks),
        //         );
        //     }
...
public function update_encode_transfer(){
    ...
    public function report_main_avail(){  
        ...
        $date=date("Y-m-d");
        if($row_avail!=0){
            foreach($this->super_model->select_join_where("et_head","et_details","damage='0' AND accountability_id = '0' AND change_location = '0' AND lost='0' AND cancelled = '0' AND obsolete='0' AND save_temp='0'","et_id") as $et){
    ...
    public function report_print_avail(){
        ...
        foreach($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE accountability_id='0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND cancelled = '0' AND save_temp='0' $filter_query GROUP BY eh.et_id ORDER BY set_id DESC") AS $et){
            $data['user_id'] = $_SESSION['fullname'];
            $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
            $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
            $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
    ...
    public function report_set_avail(){  
        ...
        if($row_set_avail!=0){
            foreach($this->super_model->select_custom_where('et_head', "accountability_id='0' AND cancelled='0'") AS $et){
                ...
                foreach($this->super_model->select_custom_where("et_details", "damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND set_id!='0' AND et_id ='$et->et_id' ORDER BY set_id ASC") AS $det){
                    $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                    $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
    ...
    public function set_print_avail_all(){  
        ...
        foreach($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE accountability_id='0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND set_id!='0' AND save_temp='0' $filter_query ORDER BY set_id DESC") AS $b){
            ...
            $count_set = $this->super_model->count_custom("SELECT eh.et_id FROM et_details ed INNER JOIN et_head eh ON eh.et_id = ed.et_id WHERE accountability_id = '0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND save_temp='0' AND set_id ='$b->set_id' $filter_query");
            ...
            $set_currency_id = $this->super_model->select_column_where("et_set","set_currency","set_id",$b->set_id);
            $set_currency=$this->super_model->select_column_where("currency","currency_name","currency_id",$set_currency_id);
            $total=$b->qty*$set_price;
            $inspection_date=array();
            $inspected_by=array();
            $ins_status=array();
            $ins_remarks=array();
            ...
            $data['details'][]=array(
                ...
                'set_name'=>$set_name,
                'set_lot'=>$set_lot,
                'unit_price'=>number_format($set_price,2)." ".$set_currency,
                'unit'=>$unit,
                'qty'=>$b->qty,
                'total'=>$total,
                'count_set'=>$count_set,
    ...
    public function report_history(){  
        ...
        foreach($this->super_model->custom_query("SELECT * FROM return_details rd INNER JOIN return_head rh ON rd.return_id=rh.return_id WHERE accountability_id='$id'") AS $r){
            ...
            $accountabilitys=$this->super_model->select_column_where("employees","employee_name","employee_id",$accountability_id);
            $return_accountability=$this->super_model->select_column_where("employees","employee_name","employee_id",$r->accountability_id);
            ...
            $obsolete = $this->super_model->select_column_where("et_details", "obsolete", "et_id", $r->et_id);
            ...
            if($count_return!=0){
                $data['sub'][] = array(
                    ...
                    'qty'=>$qty,
                    'upgrade_item'=>'',
                    'accountability'=>$return_accountability,
                    'accountabilitys'=>$accountabilitys,
                    'empid'=>$accountability_id,
                    'unit_price'=>$unit_price,
                    'lost'=>$lost,
                    'date_issued'=>$r->date_issued,
                    'date_returned'=>$r->return_date,
                    'remarks_all'=>$remarks_all,
                    'remarks'=>$r->return_remarks,
                    'damaged'=>'',
                    'incident_description'=>'',
                    'replacement'=>'',
                    'borrowed'=>$borrowed,
            ...
            foreach($this->super_model->select_row_where("damage_info","et_id",$r->et_id) AS $dam){
                //$remarks_all="Damaged - ".$dam->incident_date;
                $remarks_alls="Damaged";
                ...
                $set_id =$this->super_model->select_column_where("et_details", "set_id", "ed_id", $dam->ed_id);
            ...
            foreach($this->super_model->select_custom_where("repair_details","ed_id='$r->ed_id' AND method='0'") AS $rep){
                ...
                $set_id =$this->super_model->select_column_where("et_details", "set_id", "ed_id", $rep->ed_id);
            ...
            if($count_upgrade!=0){
                foreach($this->super_model->select_custom_where("repair_details","ed_id='$r->ed_id' AND method='1' AND remove_upgrade='0'") AS $rep){
                    ...
                    $lost = $this->super_model->select_column_where("et_details", "lost", "et_id", $rep->et_id); 
                    $obsolete = $this->super_model->select_column_where("et_details", "obsolete", "et_id", $rep->et_id); 
    ...
    public function print_history(){  
        ...
        $replacement='';
         foreach($this->super_model->custom_query("SELECT * FROM et_details ed INNER JOIN et_head eh ON ed.et_id=eh.et_id WHERE accountability_id='$id' AND eh.cancelled = '0'") AS $sub){
                ...
                $count_set = $this->super_model->count_custom("SELECT et_head.et_id FROM et_details INNER JOIN et_head ON et_head.et_id = et_details.et_id WHERE set_id ='$et_set_id'");
                $data['count_set']=$count_set;
                $set_price = $this->super_model->select_column_where("et_set","set_price",'set_id',$sub->set_id);
                $set_cur = $this->super_model->select_column_where("et_set","set_currency",'set_id',$sub->set_id);
                $set_currency = $this->super_model->select_column_where("currency","currency_name",'currency_id',$set_cur);
        ...
        foreach($this->super_model->custom_query("SELECT * FROM return_details rd INNER JOIN return_head rh ON rd.return_id=rh.return_id WHERE accountability_id='$id'") AS $r){
            ...
            $et_set_id = $this->super_model->select_column_where("et_set","set_id",'set_id',$set_id);
            // $count_set = $this->super_model->count_custom("SELECT et_head.et_id FROM et_details INNER JOIN et_head ON et_head.et_id = et_details.et_id WHERE set_id ='$et_set_id'");
            $count_set = $this->super_model->count_custom("SELECT COUNT(*) AS cnt FROM et_details WHERE set_id = '$set_id' AND et_id = '".$r->et_id."'");
            $data['count_set']=$count_set;
            $set_price = $this->super_model->select_column_where("et_set","set_price",'set_id',$set_id);
            $accountability_id = $this->super_model->select_column_where("et_head", "accountability_id", "et_id", $r->et_id);
            $accountabilitys=$this->super_model->select_column_where("employees","employee_name","employee_id",$accountability_id);
            $et_id = $this->super_model->select_column_where("return_details", "et_id", "return_id", $r->return_id);
            $unit_id =$this->super_model->select_column_where("et_head", "unit_id", "et_id", $r->et_id);
            $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $unit_id);
            $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $r->accountability_id);
            $category_id =$this->super_model->select_column_where("et_head", "category_id", "et_id", $r->et_id);
            $subcat_id =$this->super_model->select_column_where("et_head", "subcat_id", "et_id", $r->et_id);
            $category =$this->super_model->select_column_where("category", "category_name", "category_id", $category_id);
            $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $subcat_id);
            ...
            $et_desc =$this->super_model->select_column_where("et_head", "et_desc", "et_id", $r->et_id);
            $qty =$this->super_model->select_column_where("et_head", "qty", "et_id", $r->et_id);
            $date_returned =$this->super_model->select_column_where("return_head", "return_date", "return_id", $r->return_id);
            $date_issued =$this->super_model->select_column_where("return_details", "date_issued", "return_id", $r->return_id);
            $unit_price =$this->super_model->select_column_where("et_details", "unit_price", "et_id", $r->et_id);
            if(!empty($accountabilitys)){
                $remarks = $r->return_remarks;
            }else {
                if(!empty($r->return_remarks)){
                    $remarks = $r->return_remarks.' - Returned';
                }else {
                    $remarks = 'Returned';
                }
            }
    ...
    public function print_history_lost(){  
    $this->load->view('template/header');
    $this->load->view('template/navbar',$this->dropdown);
...
>>>>>>> e31b756a8097c7a8a749d1d0861ab09783d85584
    ...
    foreach($this->super_model->custom_query("
        SELECT * FROM et_details ed 
        INNER JOIN et_head eh ON ed.et_id = eh.et_id 
        WHERE eh.accountability_id = '$id' AND eh.cancelled = '0'
    ") as $sub){
...
<<<<<<< HEAD
        ...
        foreach($this->super_model->select_row_where('return_details','return_id',$ret->return_id) AS $det){
            ...
            $accountabilitys = $this->super_model->select_column_where("employees","employee_name","employee_id",$ret->accountability_id);

            if(!empty($accountabilitys)){
                $remarks = $det->return_remarks;
            } else {
                if(!empty($det->return_remarks)){
                    $remarks = $det->return_remarks.' - Returned';
                } else {
                    $remarks = 'Returned';
                }
            }
            ...
            $data['sub'][] = array(
                ...
                'set_price' => 0,
                'set_currency' => '',
                'asset_control_no' => isset($asset_control_no) ? $asset_control_no : '',
                'serial_no' => $serial,
                'currency' => isset($currency) ? $currency : '',
                'count_set' => 1,
                'cat' => '',
                ...
                'unit_price' => $price,
                'lost' => 0,
                'date_issued' => isset($date_issued) ? $date_issued : '',
                'date_returned' => $ret->return_date,
                'remarks' => !empty($det->return_remarks) ? $det->return_remarks : (!empty($data['remarks']) ? $data['remarks'] : 'Returned'),
                'replacement'=>'',
            );
        }
...
>>>>>>> e31b756a8097c7a8a749d1d0861ab09783d85584
    ...
    public function search_report(){
        ...
        $data['placement1'] = $this->super_model->select_all_order_by('placement', 'placement_name', 'ASC');
        $data['rack1'] = $this->super_model->select_all_order_by('rack', 'rack_name', 'ASC');
        foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE accountability_id!='0' AND cancelled='0' AND save_temp='0' AND lost='0' AND obsolete='0' AND $query GROUP BY eh.et_id") AS $et){
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
     ...
     public function itemlist(){
        ...
        if($rows!=0){
            ...
            foreach($this->super_model->select_custom_where("et_head", "accountability_id='0' AND cancelled='0' AND save_temp = '0' AND et_desc LIKE '%$item%'") AS $itm){
                foreach($this->super_model->select_custom_where("et_details", "damage='0' AND et_id ='$itm->et_id' AND change_location = '0' AND lost='0' AND obsolete='0'") AS $det){  
                    $qty = 1;
    ...
    public function multi_return(){
        ...
        if($row!=0){
            foreach($this->super_model->select_custom_where("et_head", "accountability_id='$id' AND cancelled='0'") AS $mul){
                ...
                foreach($this->super_model->select_custom_where("et_details", "et_id = '$mul->et_id' AND lost ='0' AND obsolete='0'") AS $det){
                    $currency = $this->super_model->select_column_where("currency", "currency_name", "currency_id", $det->currency_id);
                    $data['details'][] = array(
                        'set_id'=>$det->set_id,
                        'edid'=>$det->ed_id,
    ...
    public function export_equipment(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Y2', "Total Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z2', "Remarks");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AA2', "Date of Inspection");
        ...
        if($filter!=''){
            $previousId = '';
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE eh.accountability_id!='0' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND obsolete='0' AND $query GROUP BY eh.et_id ORDER BY ed.set_id DESC") AS $et){
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AB'.$num, implode("\n",$inspected_by));
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AC'.$num, implode("\n",$ins_status));
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AD'.$num, implode("\n",$ins_remarks));
            ...
            $previousId = '';
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE eh.accountability_id!='0' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND obsolete='0' GROUP BY eh.et_id ORDER BY ed.set_id DESC") AS $et){
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
                ...
                $et_set_id = $this->super_model->select_column_where("et_set", "set_id", "set_id", $et->set_id);
                // $count_set = $this->super_model->count_custom("SELECT et_head.et_id FROM et_details INNER JOIN et_head ON et_head.et_id = et_details.et_id WHERE accountability_id = '$et->accountability_id' AND set_id ='$et_set_id' AND save_temp='0'");
                $count_set = $this->super_model->count_custom("SELECT eh.et_id FROM et_details ed INNER JOIN et_head eh ON eh.et_id = ed.et_id WHERE accountability_id = '$et->accountability_id' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND obsolete='0' AND ed.set_id ='$et->set_id'");
                if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade==0 && $et->damage==0){
                    $status = 'Assigned';
                }else if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade!=0 && $et->damage==0){
                    $status = 'Assigned / Upgraded';
    ...
    public function insert_replace(){
     ...
     public function history_view2(){  
        ...
        foreach($this->super_model->select_row_where('et_details', 'ed_id', $id) AS $cur){
            ...
            $data['lost'] =$this->super_model->select_column_where("et_details", "lost", "ed_id", $cur->ed_id);
            $data['obsoletes'] =$this->super_model->select_column_where("et_details", "obsolete", "ed_id", $cur->ed_id);
            $location_id=$this->super_model->select_column_where("et_details", "location_id", "ed_id", $cur->ed_id);
            $data['location'] =$this->super_model->select_column_where("location", "location_name", "location_id", $location_id);
            $date_issued =$this->super_model->select_column_where("et_details", "date_issued", "ed_id", $cur->ed_id);
            foreach($this->super_model->select_row_where('et_head', 'et_id', $cur->et_id) AS $head){
    ...
    public function search_report_avail(){
        $this->load->view('template/header');
        $this->load->view('template/navbar',$this->dropdown);
        $url=$this->uri->segment(2);
        if(!empty($this->input->post('encoded_from'))){
        ...
        $data['rack1'] = $this->super_model->select_all_order_by('rack', 'rack_name', 'ASC');
        $data['avail']=array();
        foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE eh.accountability_id = '0' AND cancelled='0' AND save_temp='0' AND lost='0' AND obsolete='0' AND $query GROUP BY eh.et_id") AS $et){
            $damage =$this->super_model->select_column_where("et_details", "damage", "et_id", $et->et_id);
            $asset_control_no =$this->super_model->select_column_where("et_details", "asset_control_no", "et_id", $et->et_id);
            $serial_no =$this->super_model->select_column_where("et_details", "serial_no", "et_id", $et->et_id);
            $item =$this->super_model->select_column_where("et_head", "et_desc", "et_id", $et->et_id);
    ...
    public function export_equipment_avail(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M2', "Supplier");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N2', "Date Issued ");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O2', "Previous Owner");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('P2', "Status");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q2', "Office / Department");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R2', "Placement");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S2', "Company");
        ...
        if($filter!=''){
            $previousId = '';
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE eh.accountability_id='0' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND obsolete='0' AND damage='0' AND change_location = '0' AND $query GROUP BY eh.et_id ORDER BY ed.set_id DESC") AS $et){
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$num, $et->acquired_by);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$num, $et->date_issued);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$num, $previous);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('P'.$num, $status);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$num, $et->department);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$num, $placement);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$num, $company);
                ...
                if($et->set_id!=0 && ($previousId !== $et->set_id)){
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z'.$num, $et->remarks);
                    $objPHPExcel->getActiveSheet()->mergeCells('Z'.$num.':Z'.($num+$count_set));
                }else if($et->set_id==0){
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z'.$num, $et->remarks);
                }
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AA'.$num, implode("\n",$inspection_date));
            ...
            $previousId = '';
            foreach ($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE eh.accountability_id='0' AND eh.cancelled='0' AND eh.save_temp='0' AND lost='0' AND damage='0' AND  obsolete='0' AND change_location = '0' GROUP BY eh.et_id ORDER BY ed.set_id DESC") AS $et){
                $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $et->unit_id);
                $accountability =$this->super_model->select_column_where("employees", "employee_name", "employee_id", $et->accountability_id);
                $category =$this->super_model->select_column_where("category", "category_name", "category_id", $et->category_id);
                $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $et->subcat_id);
                ...
                $et_set_id = $this->super_model->select_column_where("et_set", "set_id", "set_id", $et->set_id);
                // $count_set = $this->super_model->count_custom("SELECT et_head.et_id FROM et_details INNER JOIN et_head ON et_head.et_id = et_details.et_id WHERE accountability_id = '$et->accountability_id' AND set_id ='$et_set_id' AND save_temp='0'");
                $count_set = $this->super_model->count_custom("SELECT eh.et_id FROM et_details ed INNER JOIN et_head eh ON eh.et_id = ed.et_id WHERE ed.set_id ='$et->set_id' AND save_temp='0' AND accountability_id = '0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0'");
                $count_set_id = $this->super_model->count_rows_where("et_details","set_id",$et_set_id);
                if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade==0 && $et->damage==0){
                    $status = 'Assigned';
                }else if($et->accountability_id!=0 && $et->borrowed==0 && $et->lost==0 && $et->upgrade!=0 && $et->damage==0){
    ...
    public function search_report_set(){
        ...
        // foreach($this->super_model->select_custom_where('et_head', "accountability_id='0' AND cancelled='0'") AS $et){
             // foreach($this->super_model->select_custom_where("et_details", "damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND set_id!='0' AND et_id ='$et->et_id' $filter_query ORDER BY set_id ASC") AS $det){
        $data['avail']=array();
        foreach($this->super_model->custom_query("SELECT * FROM et_head eh INNER JOIN et_details ed ON eh.et_id=ed.et_id WHERE accountability_id='0' AND cancelled='0' AND damage='0' AND change_location = '0' AND lost='0' AND obsolete='0' AND set_id!='0' $filter_query ORDER BY set_id ASC") AS $det){
            $unit =$this->super_model->select_column_where("unit", "unit_name", "unit_id", $det->unit_id);
            $set_id =$this->super_model->select_column_where("et_details", "set_id", "et_id", $det->et_id);
            $category =$this->super_model->select_column_where("category", "category_name", "category_id", $det->category_id);
            $subcat =$this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $det->subcat_id);
            $set_name =$this->super_model->select_column_where("et_set", "set_name", "set_id", $det->set_id);
            $data['avail'][] = array(
                'et_id'=>$det->et_id,
                'ed_id'=>$det->ed_id,
                'empid'=>$det->accountability_id,
                'unit'=>$unit,
                'asset_control_no'=>$det->asset_control_no,
                'damaged'=>$det->damage,
